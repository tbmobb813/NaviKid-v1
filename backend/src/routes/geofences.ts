...
export async function geofenceRoutes(server: FastifyInstance) {
  ...
  server.post('/', {
    ...
    if (relationshipCheck.rowCount === 0) {
      return reply.forbidden('You do not have access to create safe zones for this child');
    }
  ...
  server.patch('/:id', {
    ...
    if (result.rowCount === 0) {
      return reply.notFound('Safe zone not found or you do not have permission to update it');
    }
  ...
  server.delete('/:id', {
    ...
    if (result.rowCount === 0) {
      return reply.notFound('Safe zone not found or you do not have permission to delete it');
    }
  ...
  server.post('/events', {
    ...
    if (user.userId !== safeZone.child_id) {
      return reply.forbidden('You do not have permission to create events for this safe zone');
    }
