...
describe('parentalStore', () => {
  ...
  describe('PIN Authentication', () => {
    it('allows access when PIN is not required', async () => {
      const storedSettings = {
        requirePinForParentMode: false,
    ...
    it('allows access when no PIN is set (first-time setup)', async () => {
      SecureStore.getItemAsync.mockResolvedValue(null);
    ...
    it('sets a new PIN with secure hashing', async () => {
      const { result } = renderHook(() => useParentalStore(), { wrapper });
  ...
  describe('Check-In Requests', () => {
    it('creates a new check-in request', async () => {
      const { result } = renderHook(() => useParentalStore(), { wrapper });
    ...
    it('creates urgent check-in requests', async () => {
      const { result } = renderHook(() => useParentalStore(), { wrapper });
    ...
    it('completes a check-in request', async () => {
  ...
  describe('Device Pings', () => {
    it('sends a location ping', async () => {
      const { result } = renderHook(() => useParentalStore(), { wrapper });
  ...
  describe('Dashboard Data', () => {
    it('adds check-in to dashboard', async () => {
      const { result } = renderHook(() => useParentalStore(), { wrapper });
    ...
    it('keeps only the last 10 check-ins', async () => {
      const { result } = renderHook(() => useParentalStore(), { wrapper });
  ...
  describe('Security Features', () => {
    it('resets auth attempts after successful authentication', async () => {
      const salt = 'test_salt_123';
      const pin = '1234';
      const hashedPin = `hashed_${pin}${salt}`;
    ...
    it('persists auth attempts to storage', async () => {
      const salt = 'test_salt_123';
      const correctPin = '1234';
      const wrongPin = '5678';
      const hashedCorrectPin = `hashed_${correctPin}${salt}`;
