 ...
 *
 * Usage:
 *   LINT_TIMEOUT_MS=60000 node ./scripts/lint-with-timeout.js [eslint args...]
 *
 * If no args are provided the default eslint pattern used previously is run.
 */
...
async function main() {
  ...
  // If user provided args after script, use them; otherwise use a safe default.
  const userArgs = process.argv.slice(2);
  const defaultArgs = ['**/*.{ts,tsx}', '--max-warnings=0'];
  const args = userArgs.length ? userArgs : defaultArgs;

  // Spawn eslint with node
