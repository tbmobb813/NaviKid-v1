...
function run(command, prefix = 'run') {
  ...
  const spinInterval = setInterval(() => {
    ...
    heartbeat += 1;
    // Every 10 ticks (~1s) also print a timestamped heartbeat line to make progress obvious in logs
    if (heartbeat % 10 === 0) {
  ...
  proc.stdout.on('data', (data) => {
    // clear spinner line then print output with prefix
    try {
    ...
    data
      ...
      .split(/\r?\n/)
      .filter(Boolean)
      .forEach((line) => console.log(`${padPrefix(prefix)}${line}`));
  });
  ...
  proc.stderr.on('data', (data) => {
    ...
    data
      ...
      .split(/\r?\n/)
      .filter(Boolean)
      .forEach((line) => console.error(`${padPrefix(prefix)}ERR: ${line}`));
  });
...
if (require.main === module) {
  ...
  if (args.length === 0) {
    console.error('Usage: runner.js "<command>" [prefix]');
    process.exit(2);
  }
